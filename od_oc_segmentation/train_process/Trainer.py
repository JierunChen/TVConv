...
import tqdm
import socket
from od_oc_segmentation.utils.metrics import *
...
class Trainer(object):
    ...
    def __init__(self, cuda, model, lr, val_loader, train_loader, out, max_epoch, optim, gpu, stop_epoch=None,
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in tqdm.tqdm(
                ...
                if len(str2list(self.gpu)) > 1:
                    data = image.to(torch.device("cuda:0"))
                    target_map = label.to(torch.device("cuda:0"))
                    domain_code = domain_code.to(torch.device("cuda:0"))
                else:
                    data = image.cuda()
